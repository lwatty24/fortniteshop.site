...
import { X, Search, Plus, RotateCcw, Trash2, Pencil, Share2 } from 'lucide-react';
...
export function CollectionItemsModal({ isOpen, onClose, collection }: CollectionItemsModalProps) {
  ...
  return createPortal(
    <>
      ...
      <AnimatePresence key="edit-modal">
        {showEditModal && (
          <EditCollectionModal
      ...
      <AnimatePresence key="share-modal">
        {showShareModal && (
          <ShareCollectionModal
            isOpen={showShareModal}
            onClose={() => setShowShareModal(false)}
            collection={collection}
            onUpdate={setLocalCollection}
          />
        )}
      </AnimatePresence>
    </>,
